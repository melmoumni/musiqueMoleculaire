...
class Tenu implements Effet{
	...
	public void remplirSequenceur( Molecule mol) throws InvalidMidiDataException{
	    ...
	    Midi.noteTenue(mol.note(), mol.getVolume(), mol.getTimbre(), mol.positions.get(0).temps, mol.positions.get(mol.positions.size() - 1).temps*1000 );
	    if (!Controleur.isChercheur) {
		Midi.noteTenue(mol.note(), mol.getVolume(), mol.getTimbre2(), mol.positions.get(0).temps, mol.positions.get(mol.positions.size() - 1).temps*1000);
	    }
